function playLatch() {
  ...
  if (iframe) {
    ...
    iframe.src =
      "https://w.soundcloud.com/player/?url=https%3A//api.soundcloud.com/tracks/975902716&color=%23ff5500&auto_play=false&hide_related=true&show_comments=false&show_user=false&show_reposts=false&show_teaser=false&visual=false";
    latch.appendChild(iframe);
  }
}
function goComment() {
  var comment = confirm(
    "Beri pendapat anda tentang website ini, komentarmu bisa dilihat sama yang lain juga! Tekan OK untuk ke webnya (secreto.site)"
  );
  if (comment) {
...
function hideMenu() {
  if (abb != 1) {
    alert(
      "Ini akan menyembunyikan seluruh menu inputan, termasuk nama, NISN, hingga tombol ini. Klik di logo SNBP dibawah atau refresh halaman ini untuk menampilkan menu kembali."
    );
    abb = 1;
  }
...
function alertNote() {
  alert(
    "Web ini dibuat untuk membuat fake lolos snbp dengan semata-mata untuk tujuan for fun saja. Tidak ada data inputan apapun yang akan dikirim keluar. Harap gunakan dengan bijak! Developer tidak bertanggung jawab atas penyalahgunaan website."
  );
}
function alertLink() {
  var result = confirm(
    "Link penerimaan mahasiswa baru dapat anda cari di google karena memiliki banyak variasi, tapi jika anda malas untuk mencari, ketik saja di input box https://penerimaan.{singkatan kampus}.ac.id/. Klik OK untuk memasukkan template web ke input box."
  );
  if (result) {
    document.getElementById("ilink").value =
      "https://penerimaan.{singkatan kampus}.ac.id/";
  }
}
document
  .getElementById("dropdownBoxes")
...
function displayInput() {
  ...
  for (i = 0; i < userInputColumn.length; i++) {
    document.getElementById("iuniv").value = document
      .getElementById("iuniv")
      .value.toUpperCase();
    document.getElementById("iprodi").value = document
      .getElementById("iprodi")
      .value.toUpperCase();
    var inputText = document.getElementById(userInputColumn[i]).value;
    document.getElementById(outputSpan[i]).textContent =
      document.getElementById(userInputColumn[i]).value;
  }
  ...
  if (ddd == 0) {
    ...
    setTimeout(function () {
      alert(
        "Kalau suka sama webnya, share sama yang lain biar banyak yang tau. \nIG Author tertera di menu atas, jika ada kendala atau apapun bisa DM aja ya. \nJika menunya telah disembunyikan, klik logo SNBP untuk mengembalikannya. \nSNBP GENERATOR 2025"
      );
    }, 2000);
  }
  document.getElementById("jenjang").textContent =
    document.getElementById("dropdownBoxes").value;
}
...
populateDatalist("kabKots", arrayLists.kabKots)[
  ...
  ("iuniv", "iprodi", "iprov", "iname-kab-kot")
].forEach((inputId) => {
  document.getElementById(inputId).addEventListener("input", function () {
    let inputVal = this.value.toLowerCase();
    let options = document.querySelectorAll(
      `#${inputId.replace("Box", "")} option`
    );
...
function bantuAku() {
  var result = confirm(
    "Ayo daftar lomba, boleh buat anak kuliah atau anak SMA/ sederajat, lomba ngoding/ ui ux/ paper/ poster, mending klen daftar sekarang dah sebelum tutup, cocok buat persiapan mental sebelum SNBT ATAU KASI TAU KE TEMEN KLEN YANG JAGO NGODING (s.id)"
  );
  if (result) {
